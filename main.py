"""AI自動ブログ運営サービス - メインエントリーポイント

argparseを使ったCLIインターフェース。
各コマンドで適切なモジュールを呼び出し、結果をコンソールに表示する。

使い方:
    python main.py generate --keyword "AI活用法" --category "AI・機械学習"
    python main.py schedule
    python main.py build
    python main.py keywords --category "AI・機械学習"
...
def main():
    ...
    # schedule コマンド: スケジューラー起動
    parser_sched = subparsers.add_parser(
        "schedule",
        help="記事自動生成スケジューラーを起動する",
    )
    ...
    # keywords コマンド: キーワードリサーチ
    parser_kw = subparsers.add_parser(
        "keywords",
        help="キーワードリサーチを実行する",
    )
