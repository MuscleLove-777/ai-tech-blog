"""AI自動ブログ運営サービス - 記事自動生成・投稿スケジューラー

APSchedulerを使って指定時刻に記事を自動生成・投稿する。
"""
import json
import logging
...
class BlogScheduler:
    ...
    def start(self):
        ...
        SCHEDULE_HOURS に設定された各時刻にジョブを登録し、
        ブロッキングモードで実行を開始する。
        """
        # 設定された各時刻にジョブを登録
    ...
    def _log_execution(self, result: dict):
        ...
        ファイル名は YYYYMMDD.json 形式で、同日の実行結果はリストに追記する。
