"""AI自動ブログ運営サービス - GitHub Pagesデプロイモジュール

生成した静的サイトをGitHub Pagesにデプロイする。
gh-pagesブランチに静的ファイルをpushすることで、
GitHub Pagesが自動的にサイトを公開する。

仕組み:
  1. output/site/ の中身をgitリポジトリとして初期化
  2. gh-pagesブランチにコミット
  3. GitHubリポジトリにpush
    ...
    def __init__(self):
        ...
        if not self.repo:
            raise ValueError(
                ...
                "環境変数 GITHUB_REPO を設定してください（例: username/ai-blog）"
            )
