...
生成物: 記事ページ、インデックス、カテゴリページ、サイトマップ、RSSフィード
"""
...
class SiteGenerator:
    ...
    def build_site(self):
        ...
        処理の流れ:
        1. 出力ディレクトリを初期化
        2. articles/ からJSON記事を読み込み
        3. 各記事のHTMLページを生成
        4. インデックスページ（記事一覧）を生成
        5. カテゴリページを生成
        6. サイトマップ(sitemap.xml)を生成
        7. RSSフィード(feed.xml)を生成
        """
        print(f"[サイト生成] 開始 - 出力先: {self.output_dir}")

        # 出力ディレクトリを初期化（既存があれば削除して再作成）
        ...
        for page_num in range(1, total_pages + 1):
            ...
            if page_num == 1:
                ...
                page_dir = self.output_dir / f"page"
    ...
    def _load_articles(self) -> list:
        ...
        for filepath in sorted(ARTICLES_DIR.glob("*.json")):
            try:
                ...
                article.setdefault(
                    "slug",
                    filepath.stem,  # ファイル名（拡張子なし）をスラッグに
                )
